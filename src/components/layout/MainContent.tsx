...
import RealtimeMonitorPage from '@/app/realtime/page';
...
import DeviceManagementSection from '@/components/device-management/DeviceManagementSection';
...
export default forwardRef<MainContentRef, MainContentProps>(function MainContent({ activeMenu }, ref) {
 ...
 <div className="min-h-screen bg-gray-50" data-testid="main-content">
 {/* Table of Contents */}
 <TableOfContents 
 ...
 <p className="text-blue-100">IoT Electric Energy Management System Documentation</p>
 </div>
 ...
 {(() => {
	 ...
	 if (userItem.role === 'admin') roleClass = 'bg-red-100 text-red-800';
	 else if (userItem.role === 'manager') roleClass = 'bg-yellow-100 text-yellow-800';
	 return (
		 <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${roleClass}`}>{userItem.role}</span>
	 );
 })()}
 </td>
 <td className="px-6 py-4 whitespace-nowrap">
 {(() => {
	 ...
	 if (userItem.status === 'Active') statusClass = 'bg-green-100 text-green-800';
	 return (
		 <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${statusClass}`}>{userItem.status}</span>
	 );
 })()}
 </td>
 <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
 ...
 <div className="md:hidden space-y-3">
 {users.map((userItem) => (
 <div key={`user-card-${userItem.id}`} className="bg-white border rounded-lg p-4 shadow-sm">
 <div className="flex items-start justify-between">
 <div className="flex items-center space-x-3 flex-1">
 ...
 <p className='text-gray-500 '><strong className="text-gray-700 ">Role:</strong> {user?.role}</p>
 <p className='text-gray-500 '><strong className="text-gray-700 ">Status:</strong> Active</p>
 </div>
 </div>
 <div>
 ...
 <input type="checkbox" className="mr-2 accent-blue-600" defaultChecked />
 Email notifications
 </label>
 <label className="flex items-center text-gray-500 ">
 <input type="checkbox" className="mr-2 accent-blue-600" defaultChecked />
